...
class BallTracker:
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber('/camera_front/rgb/image_raw', Image, self.image_callback)
        self.image_pub = rospy.Publisher('/ball_tracker/image_processed', Image, queue_size=1)
    ...
    def image_callback(self, msg):
        try:
            ...
            if len(contours) > 0:
                ...
                cv2.rectangle(cv_image, (x, y), (x + w, y + h), (255, 0, 0), 2)  # The rectangle will be green and have a thickness of 2
                ...
                cv2.rectangle(cv_image, (x, y - text_size[1] - 5), (x + text_size[0], y), (255, 0, 0), -1)  # Background rectangle for the label
                
                
                cv2.putText(cv_image, label, (x, y - 5), font, font_scale, (255, 255, 255), font_thickness, lineType=cv2.LINE_AA)
            ...
            cv2.line(cv_image, (int(image_center_x), int(image_center_y) - crosshair_radius + crosshair_offset), 
                     (int(image_center_x), int(image_center_y) - crosshair_radius - crosshair_length + crosshair_offset), (0, 100, 0), 2)
            cv2.line(cv_image, (int(image_center_x), int(image_center_y) + crosshair_radius - crosshair_offset), 
                     (int(image_center_x), int(image_center_y) + crosshair_radius + crosshair_length - crosshair_offset), (0, 100, 0), 2)
            cv2.line(cv_image, (int(image_center_x) - crosshair_radius + crosshair_offset, int(image_center_y)), 
                     (int(image_center_x) - crosshair_radius - crosshair_length + crosshair_offset, int(image_center_y)), (0, 100, 0), 2)
            cv2.line(cv_image, (int(image_center_x) + crosshair_radius - crosshair_offset, int(image_center_y)), 
                     (int(image_center_x) + crosshair_radius + crosshair_length - crosshair_offset, int(image_center_y)), (0, 100, 0), 2)
